...
async function fetchFearGreedIndex() {
  try {
    ...
    console.warn(`Fear & Greed fetch failed: ${err.message} — omitting this signal`);
    return null;
  }
}
...
async function fetchNewsHeadlines(query, limit = 5) {
  ...
  // Headlines are a supporting layer, never load-bearing: any failure (network
  // error, invalid key, rate limit) degrades to [] and is logged. It must never
  // throw, or a broken news upstream would take the whole stress test down with
  // it and hide the base rates the user actually came for.
  try {
    ...
    if (!res.ok) {
      console.warn(`NewsAPI returned HTTP ${res.status} for "${query}" — omitting headlines this run`);
      return [];
    }
    const data = await res.json();
    return (data.articles || []).map(a => ({ title: a.title, source: a.source.name, publishedAt: a.publishedAt }));
  } catch (err) {
    console.warn(`Headline fetch failed for "${query}": ${err.message} — omitting headlines this run`);
    return [];
  }
}
